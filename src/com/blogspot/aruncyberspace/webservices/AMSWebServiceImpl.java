...
public class AMSWebServiceImpl {
	...
	public String Authenticate(@PathParam("username") String username,
		...
		try {
			...
			String coursesquery = "SELECT c.CourseId, CourseName, CourseStartTime, CourseEndTime from courses c inner join enrolled e on e.courseId = c.courseId inner join login l on l.userId = e.StudentId where l.userId =(select UserId from login where MavEmail='" + user.getMavEmail() + "') ";
